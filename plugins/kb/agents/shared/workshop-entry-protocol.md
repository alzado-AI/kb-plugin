...
Shared entre `/program` y `/project`. Al entrar al workshop — en CUALQUIER estacion — ejecutar SIEMPRE estos 2 pasos.
...
**Si el workshop es `/project`:** leer la seccion `project_tabs` de `base_file_content` como `PROJECT_TEMPLATE` — contiene tabs, `content_scaffold` por tab y metodologia declarada para projects.
...
- Si existe → extraer `doc_id` y luego `kb google doc list-tabs DOC_ID`. Guardar como `DOC_STATE`.
  - Para `/project`: filtrar tabs de ESTE project → `DOC_STATE`.
- Si no existe doc → en `/program` se crea en SETUP INICIAL; en `/project`, no se puede escribir contenido sin doc del program.
